/*! Liberia2USA Express - Bridging Nations - WITH IMPROVED LOGIN */
(function() {
  'use strict';
  ...
  function showLoginPage() {
    ...
    // Check if there are demo users available
  ...
  function handleLogin(event) {
    ...
    .then(response => {
      if (!response.ok) {
        ...
        if (tryDemoLogin()) {
          return; // Demo login successful, exit the promise chain
        }
    ...
    .catch(error => {
      ...
      // If backend failed and demo login wasn't attempted yet, try it
  ...
  document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 Liberia2USA Express loaded - WITH IMPROVED LOGIN');
    showPage('home');
  });
